...
#### Содержание 
[Календарь](#CalendarDates)

[Регионально-независимое время](#LocationIndependentTimes)

[Регионально-зависимое время](#LocationDependentTimes)

[Форматированный ввод и вывод](#FormattedInputAndOutput)

Библиотека [Boost.DateTime](http://www.boost.org/doc/libs/1_62_0/doc/html/date_time.html) может быть использована для обработки временных данных, таких как календарь и время. Кроме того, Boost.DateTime предоставляет расширения для учета часовых поясов и поддерживает форматированный ввод и вывод календаря и времени. Если вы ищете функции для получения текущего времени или измерения текущего времени, смотри Boost.Chrono в [Главе 37][chapter37]

---
<a name="CalendarDates"></a>
#Календарные даты 
Boost.DateTime поддерживает только кадендарь на основе Григорианского календаря, что не является проблемой, так как это наиболе наиболее широко используемый календарь. Если вы организовываете встречу с кем-либо 12 мая 2014 года, то вам не нужно говорить, что эта дата основывается на Григорианском календаре. 

Григорианский календарь был введен папой Григорием XIII в 1582. Boost.DateTime поддерживает календарные даты с 1400 до 9999 годов, что означает, что поддержка ведется с 1582 года. Таким образом вы можете использовать Boost.DateTime для любой календарной даты после 1400 года до тех пор, пока дата преобразуется в Григориансокм календаре. Для обрабоки более ранней даты, Boost.DateTime должен быть переведен на новый календарь. 

Заголовочный файл `boost/date_time/gregorian/gregorian.hpp` содержит определения для всех классов и функций, которые обрабатывают календарь. Эти функции и классы могут быть найдены в пространстве имен `boost::gregorian`. Чтобы создать дату, используйте класс `boost::gregorian::date`. 
...
Как показано в [примере 36.1](#Example36.1) существует множество доступных функций-членов. Некоторые функции-члены, такие как `year()`, `month()` и `day()`, возвращают соответствующие компоненты даты и другие вычисляемые значения, такие как `day_of_week()` и `end_of_month()`. 

Конструктор `boost::gregorian::date` ожидает числовые значения для года, месяца и дня, чтобы установить дату. Тем не менее, программа выводит **Jan** - месяц, **Fri** - день недели. Возвращаемые значения `month()` и `day_of_week()` являются не постоянными числовыми значениями, а значеними типа `boost::gregorian::date::month_type` и `boost::gregorian::date::day_of_week_type`. Boost.DateTime обеспечивает полную поддержку форматированного ввода и вывода, поэтому можно настроить выход, например, с **Янв** по **1**.
...
Пространство имен `boost::gregorian` содержит свободно-стоящие функции для преобразования даты, хранящуюся в формате строки в объект типа `boost::gregorian::date`. [Пример 36.2](Example36.2) преобразует дату в формат ISO 8601, используя функцию `boost::gregorian::date_from_iso_string()`. Еще существуют функции: `boost::gregorian::from_simple_string()` и `boost::gregorian::from_us_string()`.

В то время как `boost::gregorian::date` обозначает конкретное время, `boost::gregorian::date_duration` обозначает длительность. 
...
Ни `boost::gregorian::months`, ни `boost::gregorian::years` позволяет определить количество дней, потому что месяцы и годы имеют разную длинну. Тем не менее, использование этих классов, как показано в [Примере 36.5](#Example36.5), имеет смысл.
...
`boost::gregorian::date_period` предоставялет дополнительные функции-члены для таких операций, как сдвиг периода и вычисление пересечения двух перекрывающихся периодов.

Boost.DateTime также предоставляет итераторы и другие полезные, свободностоящие, функции, как показано в [Примере 36.9](#Example36.9)
...
#Регионально-независимое время

Класс `boost::posix_time::ptime` определяет регионально-независимое время. Он сохраняет время, используя `boost::gregorian::date`. Чтобы использовать `boost::posix_time::ptime`, подключите заголовочный файл `boost/date_time/posix_time/posix_time.hpp`. 
...
Подобно тому, как конструктор по умолчанию `boost::gregorian::date` создает некорректную дату, конструктор по умолчанию boost::posix_time::ptime создает некорректное время. Некорректное время также может быть создано явно, передавая `boost::date_time::not_a_date_time` в конструктор. Boost.DateTime заставляет свободно-стоящие функции и функции-члены создавать время аналогично тому, как создаются календарные даты типа `boost::gregorian::date`. 
...
`hours()`, `minutes()` и `seconds()` возвращают соответствующие части продолжительности времени, в то время, как функции-члены, такие как `total_seconds()`, которые возвращают общее количество секунд, предоставляют дополнительную информацию (смотри [Пример 36.12](#Example36.12)). Здесь нет верхнего предела, например 24 часа, к значения вы можете перейти, используя `boost::posix_time::time_duration`. 
...
Программы, описанные в данной главе, выводили результат в формате **2014-май-12**. Boost.DateTime позволяет отображать результаты в различных форматах. Дата и время могут быть отформатированы с использованием `boost::date_time::date_facet` и `boost::date_time::time_facet`. 

Boost.DateTime использует концепцию стандартных локалей. Для форматирования даты календаря, объект типа `boost::date_time::date_facet` должен быть создан и установлен в пределах местности. Строка, описывающая новый формат передается конструктору `boost::date_time::date_facet`. [Пример 36.20](#Example36.20) передает “%A, %d %B %Y”, которые указывают на то, что день недели и дата указаны полностью: **Понедельник**, **12 Мая 2014**.
